...
from utils.conftest import (
    ...
    creado_por, tipo_medida_name, delete_tipo_medida_if_exists, tipo_medida_test
)
...
class TestTipoMedidasEndpoints():
    ...
    async def test_fiscalizador_post_tipos_medidas_success(self, client):
        headers = get_basic_auth_header(email_fiscalizador, password_fiscalizador)
        response = client.post("/auth/token", headers=headers)
        access_token = response.json()["access_token"]
        headers = {"Authorization": f"Bearer {access_token}"}
    ...
    async def test_organismo_sectorial_post_tipos_medidas_success(self, client):
        headers = get_basic_auth_header(email_organismo_sectorial, password_organismo_sectorial)
        response = client.post("/auth/token", headers=headers)
        access_token = response.json()["access_token"]
        headers = {"Authorization": f"Bearer {access_token}"}
    ...
    async def test_fiscalizador_delete_tipo_medida_success(self, client, tipo_medida_test):
        headers = get_basic_auth_header(email_fiscalizador, password_fiscalizador)
        response = client.post("/auth/token", headers=headers)
        access_token = response.json()["access_token"]
        headers = {"Authorization": f"Bearer {access_token}"}
    ...
    async def test_organismo_sectorial_delete_tipo_medida_success(self, client, tipo_medida_test):
        headers = get_basic_auth_header(email_organismo_sectorial, password_organismo_sectorial)
        response = client.post("/auth/token", headers=headers)
        access_token = response.json()["access_token"]
        headers = {"Authorization": f"Bearer {access_token}"}
