...
class TestFrecuenciasEndpoints():
    async def test_admin_get_frecuencias_success(self, client):
    ...
    async def test_crud_frecuencia_success(self, client):
        ...
        try: 
            frecuencia = get_or_create("/frecuencias/", {
                "frecuencia": frecuencia_name,
            }, "frecuencia", "id_frecuencia", access_token, "frecuencia")
            assert frecuencia["frecuencia"] == frecuencia_name
            assert frecuencia["id_frecuencia"] is not None

            headers = {"Authorization": f"Bearer {access_token}"}
            response = client.put(f"/frecuencias/{frecuencia['id_frecuencia']}", headers=headers, json={
                "frecuencia": "frecuencia_test_editada",
            })
            assert response.status_code == 200
            frecuencia_editada = response.json()["frecuencia"]
            assert frecuencia_editada["frecuencia"] == "frecuencia_test_editada"
