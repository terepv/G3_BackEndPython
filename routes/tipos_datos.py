...
from shared.schemas import TipoDatoCreate, TipoDatoOut, UsuarioOut
...
@router.get(
    "/",
    response_model=list[TipoDatoOut],
...
def read_tipo_datos(
    ...
    Requiere ser usuario Admin, Fiscalizador u Organismo Sectorial para acceder a este recurso.
    """
...
def read_tipo_datos(
    ...
    """
    tipo_dato = db.query(TipoDato).filter(TipoDato.id_tipo_dato == id_tipo_dato, TipoDato.eliminado_por == None).first()
    if not tipo_dato:
...
def add_tipo_dato(
    ...
    if db.query(TipoDatoResponse).filter(TipoDatoResponse.tipo_dato.ilike(tipo_dato.tipo_dato), TipoDatoResponse.id_tipo_dato != id_tipo_dato).first():
        raise HTTPException(status_code=409, detail="Tipo de dato ya existe")
    


    tipo_dato.fecha_actualizacion = get_local_now_datetime()
    tipo_dato.tipo_dato = tipo_dato.tipo_dato
    tipo_dato.actualizado_por = user.email
    db.commit()

    return {"message": "Se actualizó tipo de dato", "Tipo de dato": tipo_dato}
